...
router.get('/:channel', function (req, res) {
    var channel = STATION.channels.get(req.params.channel);
    var channel_api_base = 'http://' + CONFIG.server.host + ':' + CONFIG.server.port + '/channel/' + channel.id;
    var out = {
        id: channel.id,
        name: channel.name,
        api: [
...
router.get('/:channel/skip', function (req, res) {
    var channel = STATION.channels.get('id', req.params.channel);
    if (!channel) {
        res.send('404');
        return;
    }
